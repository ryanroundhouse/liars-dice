...
export function claim(req: Request, res: Response){
    ...
    logger.log('info', `got request to make a claim ${JSON.stringify(currentClaim)} in ${gameId} from ${req.session.userId}`);
    ...
    if (lastMessage.messageType === MessageType.Claim){
        logger.debug(`comparing ${JSON.stringify(lastMessage)} to ${JSON.stringify(currentClaim)} by ${req.session.userId}`);
    ...
    // pass it on to the next player.
        ...
        currentClaim.message.playerId = req.session.userId;
        res.send({ result: 'OK', message: 'true' });
        logger.verbose(`gamePopulation is now:`);
        Game.gamePopulation.forEach((val, key) => logger.verbose(`${key}: ${JSON.stringify(val)}`));
        sendGameMessage(gameId, MessageType.Claim, currentClaim.message);
    }
}
